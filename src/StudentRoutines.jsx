      ...
      <main className="max-w-4xl mx-auto p-4 md:p-6 mt-4">
        ...
        {loading ? (
          ...
          routines.length === 0 ? (
            ...
            <div className="space-y-6">
              {routines.map((routine) => (
                <div key={routine.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-lg">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-800 pb-4 mb-4 gap-4">
                    <div>
                      ...
                      <h2 className="text-xl font-bold text-white mt-2 leading-tight">{routine.title}</h2>
                    </div>
                      ...
                      <tbody className="divide-y divide-slate-800/50 text-slate-300 text-sm">
                        {routine.exercises.map((ex, idx) => (
                          <tr key={idx} className="hover:bg-slate-800/20 transition-colors">
                            ...
                            <td className="py-3.5 text-center">{ex.sets}</td>
                            <td className="py-3.5 text-center">{ex.reps}</td>
                            <td className="py-3.5 text-center text-slate-400">{ex.rest_time || '-'}</td>
                            <td className="py-3.5">
                              {ex.youtube_url ? <a href={ex.youtube_url} target="_blank" rel="noreferrer" className="text-amber-500 hover:underline font-medium">Ver ↗</a> : <span className="text-slate-600">N/A</span>}
                            </td>
                          </tr>
                        ))}
                      </tbody>
          ...
          logs.length === 0 ? (
            ...
            <div className="space-y-6">
              {logs.map((log) => (
                <div key={log.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-md">
                  ...
                  {log.feedback && (
                    <div className="mb-5 bg-amber-500/10 border-l-2 border-l-amber-500 rounded-r-xl p-4">
                      ...
                      <p className="text-slate-200 italic text-sm">"{log.feedback}"</p>
                    </div>
                  )}
                  ...
                  <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 md:p-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                      {log.exercises.map((exercise) => {
                        ...
                        return (
                          <div key={exercise.index} className="flex justify-between items-center border-b border-slate-800/60 pb-2 text-sm">
